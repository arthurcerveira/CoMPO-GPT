...
class StackAugmentedRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, layer_type='GRU',
        ...
        if layer_type not in ['GRU', 'LSTM']:
            raise ValueError('Layer type must be GRU or LSTM')
    ...
    def fit(self, data, n_iterations, all_losses=[], print_every=100,
            plot_every=10, augment=False, patience=10, min_delta=1e-4):
        """
        ...
        for epoch in trange(1, n_iterations + 1, desc='Training in progress...'):
            ...
            if epoch % print_every == 0:
                print(f'[{time_since(start)} ({epoch} {epoch/n_iterations*100:.0f}%) {loss:.4f}]')
                print(self.evaluate(data=data, prime_str = '<', predict_len=100), '\n')

            if epoch % plot_every == 0:
                avg_loss = loss_avg / plot_every
                ...
                if best_loss - avg_loss > min_delta:
                    ...
                    epochs_no_improve += 1
                    print(f"No improvement in {epochs_no_improve} iterations.")

                if epochs_no_improve >= patience:
                    print(f"\nEarly stopping triggered at iteration {epoch}. "
                        f"No improvement in {patience * plot_every} iterations.")
                    break
