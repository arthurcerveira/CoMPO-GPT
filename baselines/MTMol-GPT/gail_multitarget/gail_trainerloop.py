...
class GAILTrainerLoop:
    ...
    def train(self, scenario, predictor1, predictor2):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            self.dataset2_train_loader = DataLoader(list(self.dataset2[0]), shuffle=True, pin_memory=True,
                                                    batch_size=self.batch_size, drop_last=True)
    ...
    def train_epoch(self, epoch):
        ...
        iter_train_dataloader2 = iter(self.dataset2_train_loader)
        for step in range(999):
            ...
            while buffer_count < self.ppo_buffer_size:
                try:
                    ...
                    batch2 = next(iter_train_dataloader2)
                except StopIteration:
                    return
                self.collect_samples(batch1, batch2)
            ...
            for i in range(self.n_gail):
                ...
                for mini_batch in self.replay_buffer.iterate_sample(self.ppo_mini_batch_size):
                    states1, states2, actions, action_log_probs, rewards = zip(*mini_batch)
                    gail_loss = self.reward_func.fit(states1, states2, actions)
    ...
    @torch.no_grad()
    def collect_samples(self, batch_target1, batch_target2):
        num_one_demos = int(len(batch_target1) * self.mix_demo_ratio)
        # Update Buffer for Demos sequences
        if num_one_demos > 0:
            batch_demo1 = batch_target1[:num_one_demos]
            batch_demo2 = batch_target2[:num_one_demos]
            batch = {}
            batch_idx = [torch.tensor(self.vocab.encode(string, add_bos=True, add_eos=True), dtype=torch.long,
                                      device=self.device) for string in batch_demo1]
            ...
            rewards = np.ones((len(batch_demo1))) * 2.0
            self.replay_buffer.update_batch(
                states1=batch_demo1, states2=batch_demo1,
                actions=batch_demo1,
            ...
            batch = {}
            batch_idx = [torch.tensor(self.vocab.encode(string, add_bos=True, add_eos=True), dtype=torch.long,
                                      device=self.device) for string in batch_demo2]
            batch['index'] = pad_sequence([t[:-1] for t in batch_idx], batch_first=True,
                                          padding_value=self.vocab.pad)
            batch['label'] = pad_sequence([t[1:] for t in batch_idx], batch_first=True,
                                          padding_value=self.vocab.pad)
            ...
            log_probs = self.model.generator.compute_log_probs(batch)["log_probs"]
            demos_log_probs = log_probs.tolist()
            # rewards = self.reward_func.get_reward(batch_demo1, batch_demo2, batch_demo2).tolist()
            # rewards = np.array(rewards)
            rewards = np.ones((len(batch_demo2))) * 2.0
            self.replay_buffer.update_batch(
                states1=batch_demo2, states2=batch_demo2,
                actions=batch_demo2,
                action_log_probs=demos_log_probs,
                rewards=rewards
            )
        ...
        actual_sample_size = len(batch_target1) - num_one_demos * 2
        select_batch_target1 = batch_target1[num_one_demos:num_one_demos + actual_sample_size]
        select_batch_target2 = batch_target2[num_one_demos:num_one_demos + actual_sample_size]
        ...
        rewards = self.reward_func.get_reward(select_batch_target1, select_batch_target2, sequences).tolist()
        rewards = np.array(rewards)
        self.replay_buffer.update_batch(
            states1=select_batch_target1, states2=select_batch_target2,
            actions=sequences,
