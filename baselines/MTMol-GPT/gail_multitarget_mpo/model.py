...
class SmilesGAILModel:
    def __init__(self, config, vocab, num_objectives=5):
        ...
        self.discriminator = Discriminator(disc_config, vocab, self.device, num_objectives=self.num_objectives)
...
class RnnNet_DLGN(nn.Module):
    def __init__(self, config, vocab, device, num_objectives=5):
        ...
        self.num_dir = 1 + int(config.bidirectional)
        ...
        self.hidden2outs = nn.ModuleList([
            nn.Sequential(
                nn.Linear(2 * self.num_dir * self.blstm_dim, self.blstm_dim),
                nn.Sigmoid(),
                nn.Dropout(p=0.3),
                nn.Linear(self.blstm_dim, self.blstm_dim),
                nn.Sigmoid(),
                nn.Dropout(p=0.3),
            )
            for _ in range(self.num_objectives)
        ])
        self.double_outs = nn.ModuleList([
            nn.Sequential(
                nn.Linear(2 * self.hidden_size, self.blstm_dim),
                nn.Sigmoid(),
                nn.Dropout(p=0.3),
            )
            for _ in range(self.num_objectives)
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.blstm_dim * self.num_objectives, self.out_dim),
            nn.Sigmoid(),
            nn.Dropout(p=0.3),
        )
    ...
    def init_hidden(self, batch_size):
        return torch.zeros(self.num_layers * self.num_dir, batch_size, self.hidden_size, device=self.device)

    def forward(self, demo_inputs, gen_input):
        """
        Args:
            demo_inputs: list of tensors, each with shape [batch_size, seq_len]
                containing the demonstrations for each objective.
            gen_input: tensor with shape [batch_size, seq_len] containing the
                generated samples.
        """
        if len(demo_inputs) != self.num_objectives:
            raise ValueError(f"Expected {self.num_objectives} demonstrations, got {len(demo_inputs)}")

        batch_size = gen_input.size(0)
        gen_emb = self.embeddings(gen_input.long())

        pair_features = []
        for idx in range(self.num_objectives):
            demo = demo_inputs[idx]
            if demo.size(0) != batch_size:
                raise ValueError("Each demonstration batch must match the generated batch size.")

            demo_emb = self.embeddings(demo.long())

            hidden_demo = self.init_hidden(demo.size(0))
            hidden_gen = self.init_hidden(batch_size)

            gru = self.grus[idx]
            _, demo_state = gru(demo_emb, hidden_demo)
            _, gen_state = gru(gen_emb, hidden_gen)

            demo_state = demo_state.permute(1, 0, 2).contiguous().view(batch_size, -1)
            gen_state = gen_state.permute(1, 0, 2).contiguous().view(batch_size, -1)

            demo_out = self.hidden2outs[idx](demo_state)
            gen_out = self.hidden2outs[idx](gen_state)

            pair = torch.cat([demo_out, gen_out], dim=1)
            pair = self.double_outs[idx](pair)
            pair_features.append(pair)

        logits = torch.cat(pair_features, dim=1)
        out = self.fc(logits)
        return out
...
class Discriminator:
    def __init__(self, config, vocab, device, num_objectives=5):
        self.device = device
        self.num_objectives = num_objectives
        self.model = RnnNet_DLGN(config, vocab, self.device, num_objectives=self.num_objectives).to(self.device)
        opt_config = config.optimization
    ...
    def compute_reward_gru(self, demo_inputs, gen_input):
        out = self.model.forward(demo_inputs, gen_input)
