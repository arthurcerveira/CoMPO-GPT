...
def sample_parser(parser):
    ...
    # batch size: how many molecules to sample at once
    opt_runtime.add_argument("--batch_size",
        default=1,
        type=int,
        help='Number of molecules to sample at once.')
    
    opt_runtime.add_argument("--score_samples",
        default=False,
        action="store_true",
        help="Score samples and output to file.")
...
def sample_main(args):
    ...
    while c < args.n_molecules:
        s = model.sample(args.batch_size)
        # Uncomment to canonicalize and remove invalid SMILES
        # s = canonicalize_list(s)
        # if len(s) == 0:
        #     continue

        if c + len(s) > args.n_molecules:
            s = s[:args.n_molecules - c]

        samples_smiles.extend(s)
        c += len(s)
    
    if args.score_samples:
        r = get_raw_scores(samples_smiles, scorers, scoring_function)
        if args.filter:
