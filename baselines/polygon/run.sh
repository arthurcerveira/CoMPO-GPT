...
output_dir="../../../generated_molecules/polygon-mma"
mkdir -p ${output_dir}
...
output_dir="../../../generated_molecules/polygon-le"
mkdir -p ${output_dir}

# Iterate over the list (ligand efficiency)
for disease in ${disease_list[@]}; do
    (
        polygon generate \
          --model_path ./checkpoints/model.pt --scoring_definition ../data/${disease}-le.csv --max_len 100 \
          --n_epochs 200 --mols_to_sample 8192 --optimize_batch_size 512  --optimize_n_epochs 2 --keep_top 4096 \
          --opti gauss --outF ./checkpoints/${disease}_le --device cuda  --n_jobs 1 --debug  # --save_payloads

        polygon sample \
          --model_path ./checkpoints/${disease}_le/GDM_200.pt --scoring_definition ../data/${disease}-le.csv \
          --n_molecules 10000 --output ${output_dir}/${disease}_le.csv --device cuda --debug --batch_size 1000
    )  # &
done
