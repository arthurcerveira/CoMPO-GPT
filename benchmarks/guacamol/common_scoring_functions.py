...
# Uses Chemprop for target response scoring
import sys
from pathlib import Path
current_file_dir = Path(__file__).resolve().parent

chemprop_path = current_file_dir / ".." / ".." / "qsar"
sys.path.append(str(chemprop_path))
from assessment.chemprop_callback import (
    load_model_from_target,
...
class TargetResponseScoringFunction(BatchScoringFunction):
    ...
    def __init__(self, target, score_modifier: ScoreModifier = None, output_range: Tuple[float, float] = (1, 10)) -> None:
        """
        Args:
        ...
        # Avoid loading the model every time we score a molecule
        self.model = load_model_from_target(target)
        self.target = target
        self.min_range, self.max_range = output_range

    def raw_score_list(self, smiles_list: List[str]) -> List[float]:
        # We don't need to convert SMILES to RDKit molecules anymore
        # mols = [smiles_to_rdkit_mol(smiles) for smiles in smiles_list]
        predicted_activity_proba = predict_activity_chemprop(smiles_list, self.target, self.model)
        # Normalize the predictions to 0-1 range
        predicted_activity_proba = (
            predicted_activity_proba - self.min_range
        ) / (self.max_range - self.min_range)
    
        return predicted_activity_proba


class BBBResponseScoringFunction(BatchScoringFunction):
    """
    Scoring function that measures the response of a molecule against the BBB.
    """

    def __init__(self, score_modifier: ScoreModifier = None) -> None:
        super().__init__(score_modifier=score_modifier)
        self.model = load_model_from_target("BBB")

    def raw_score_list(self, smiles_list: List[str]) -> List[float]:
        predicted_activity_proba = predict_bbb_chemprop(smiles_list, self.model)
        return predicted_activity_proba
