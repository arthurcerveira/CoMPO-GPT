...
for combination in multitarget_combination:
    ...
    for path in mols_activity_paths:
        ...
        for model in models:
            if f"{model}_Activity" in activity_df.columns:
                print(f"Skipping {model} - Activity")
                continue
            ...
            activity_df[f"{model}_Activity"] = np.array(preds).flatten()

        for model in models:
            if f"{model}_inhibition" in activity_df.columns:
                print(f"Skipping {model} - Inhibition")
                continue
